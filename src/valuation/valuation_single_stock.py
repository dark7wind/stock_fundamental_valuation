...
def valuation_single_stock(ticker, input_config_dir, input_config_file):
    ...
    ## default assumption
    flag_cost_capital_terminal_override = input_manual[
    ...
    try:
        ...
        if finance_input == "Yahoo_trailing_12_month":
            (
                ...
                df_balance_sheet,
                df_stock_statics,
            ) = get_input_finance_func(ticker, read_from_sql=True, read_TTM=False)
            df_income_statement_yearly = df_income_statement.loc[
            ...
            df_income_statement_latest = df_income_statement.loc[
                df_income_statement["endDate"] == df_income_statement["endDate"].max()
            ]
            income_statement_latest_type = df_income_statement_latest.iloc[0]["type"]
            if income_statement_latest_type == "yearly":
                df_income_statement_current = df_income_statement.loc[
                    df_income_statement["endDate"]
                ...
                df_income_statement_quarterly = df_income_statement.loc[
                ...
                if quarterly_report_number < 5:
                    ...
                    numerica_columns = df_income_statement_quarterly.select_dtypes(
                        include=np.number
                    ).columns.to_list()
                    ## sum of each quaterly results and multiple the scaling factors
                    quarterly_sum = df_income_statement_quarterly[numerica_columns].sum(
                        axis=0
                    )
                    ...
                    df_income_statement_current = df_income_statement_quarterly.loc[
                        df_income_statement_quarterly["endDate"]
                        == df_income_statement_quarterly["endDate"].max()
                    ]
                    df_income_statement_current[
                        numerica_columns
                    ] = quarterly_sum_scaling
                    ...
                    df_income_statement_yearly_latest = df_income_statement_yearly.loc[
                        df_income_statement_yearly["endDate"]
                        == df_income_statement_yearly["endDate"].max()
                    ]
                    income_statement_yearly_latest_date = (
                        df_income_statement_yearly_latest.iloc[0]["endDate"]
                    )
                    ...
                    df_income_statement_quarterly_this_year = (
                        df_income_statement_quarterly.loc[
                            df_income_statement_quarterly["endDate"]
                            > income_statement_yearly_latest_date
                        ]
                    )
                    ...
                    this_year_sum = df_income_statement_quarterly_this_year[
                        numerica_columns
                    ].sum(axis=0)
                    df_income_statement_this_year_sum = (
                        df_income_statement_quarterly_this_year.loc[
                    ...
                    ### sort by endDate
                    df_income_statement_quarterly_last_year_full = (
                        df_income_statement_quarterly_last_year_full.sort_values(
                            by=["endDate"], ascending=False
                        )
                    )
                    df_income_statement_quarterly_last_year = (
                        df_income_statement_quarterly_last_year_full.iloc[
                            4 - number_quarters : 4
                        ]
                    )
                    last_year_sum = df_income_statement_quarterly_last_year[
                        numerica_columns
                    ].sum(axis=0)
                    df_income_statement_last_year_sum = (
                        df_income_statement_quarterly_last_year.loc[
                    ...
                    trailing_12_month = (
                        df_income_statement_yearly_latest[numerica_columns].iloc[0]
            ...
            df_balance_sheet_current = df_balance_sheet.loc[
                df_balance_sheet["endDate"] == df_balance_sheet["endDate"].max()
            ]
            ...
            (
                ...
                df_balance_sheet,
                df_stock_statics,
            ) = get_input_finance_func(ticker, read_from_sql=True, read_TTM=True)
            df_income_statement_current = df_income_statement.loc[
                df_income_statement["LastUpdatedDate"]
            ...
            income_before_tax = (
                df_income_statement_current["PretaxIncome"].iloc[0] * 1000
            )
            income_tax_expense = (
                df_income_statement_current["TaxProvision"].iloc[0] * 1000
            )
            interest_expense = (
                df_income_statement_current["InterestExpense"].iloc[0] * 1000
            )
            # interest_income = df_income_statement_current['InterestIncome'].iloc[0] * 1000
        elif finance_input == "manual":
            _, _, df_stock_statics = get_input_finance_func(
                ticker, read_from_sql=True, read_TTM=True
            )
        ...
        ## outstanding shares
        df_stock_statics_current = df_stock_statics.loc[
            df_stock_statics["lastUpdatedDate"]
        ...
        effective_tax_rate = effective_tax_rate_func(
            income_before_tax,
            income_tax_expense,
            marginal_tax_rate,
            estimate_effective_tax_rate=0.25,
            flag_avg=False,
        )
        ...
        if margin_input == "manual":
            ...
            margin_historical_mean, margin_historical_recent = get_historical_margin(
                ticker
            )
        ...
        print(
            f"margin next year: {margin_next_year}, margin target: {margin_target}, converge year: {converge_year}"
        )
        ...
        ## growth list
        growth_list = growth_list_direct_func(
            r_gr_next,
            r_gr_high,
            length_high_growth,
            length_high_growth_stable,
            r_riskfree,
            flag_gr_terminal_direct,
            r_gr_terminal_direct,
        )
        print(f"growth_list: {growth_list}")

        ## margin list
        margin_list = margin_list_direct_func(
            margin_next_year, margin_target, converge_year, length_high_growth
        )
        ...
        sales_to_capital_list = sales_to_capital_list_func(
            total_revenue,
            invested_capital,
            sales_to_capital_us,
            sales_to_capital_global,
            length_high_growth,
        ...
        ## tax rate list
        tax_rate_list = tax_rate_list_func(
            effective_tax_rate,
            marginal_tax_rate,
            length_high_growth,
            length_high_growth_stable,
        ...
        nol_list = net_income_list_loss_func(
            net_income_loss_previous, ebit_list, length_high_growth, nol_initial_flag
        )
        ...
        cost_capital_list = cost_of_capital_list_func(
            cost_capital,
            length_high_growth,
            length_high_growth_stable,
            r_riskfree,
            mature_ERP,
            risk_free_terminal_override,
            cost_capital_terminal_override,
            flag_risk_free_terminal_override,
            flag_cost_capital_terminal_override,
        )
        print(f"cost of capital list: {cost_capital_list}")

        ## present value -> growth period
        pv_growth, present_value_list = present_value_growth_period_func(
            revenue_list,
            ebit_list,
            growth_list,
            margin_list,
        ...
        pv_terminal = present_value_terminal_func(
            revenue_list,
            growth_list[-1],
            margin_list,
            tax_terminal,
        ...
        operating_assets_value = value_operating_assets_func(
            pv_terminal, pv_growth, prob_failure, proceeds_failure
        )
        ...
        equity_value = equity_value_common_stock_func(
        ...
        ## estimated value
        estimated_value, price_to_value = estimated_value_share_func(
            equity_value, num_share, price_current
        )
