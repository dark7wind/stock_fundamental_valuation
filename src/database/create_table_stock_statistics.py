...
import datetime
import yaml
from src.data.download_stock_statistics import download_stock_statistics
from definitions import DATABASE_CONFIG_DIR


def insert_statistics_data_into_db():
    ...
    df = download_stock_statistics()

    ## temp solution To DO
    df = df[['Shares Outstanding 5', 'ticker', 'stockId']]
    df = df.rename(columns={"Shares Outstanding 5": "sharesOutstanding"})
...
if __name__ == '__main__':
    insert_statistics_data_into_db()